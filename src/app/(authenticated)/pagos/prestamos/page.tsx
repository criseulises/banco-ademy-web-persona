    ...
    <div>
      ...
      <div className="relative" ref={ref}>
        ...
        >
          <span style={{ color: selected ? colors.textPrimary : colors.textHint }}>
            {selected
              ? `${selected.nickname || selected.purpose} - ${selected.loanType}`
              : placeholder}
          </span>
          <svg
            width="20"
          ...
          >
            {loans.map((loan) => (
              ...
              >
                ...
                <div className="text-xs mt-0.5" style={{ color: colors.textSecondary }}>
                  {loan.loanType} · Saldo: {formatCurrency(loan.outstandingBalance)}
                </div>
              </button>
            ))}
          </div>
...
export default function PagosPrestamos() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/mock_data/loans.json').then((r) => r.json()),
    ])
      .then(([accountsData, loansData]) => {
        ...
        // Solo mostrar préstamos activos del usuario
        setLoans(
          loansData.loans.filter(
            (loan: Loan) => loan.userId === 'user_001' && loan.status === 'ACTIVE'
          )
        );
        setLoading(false);
      })
  ...
  const destinoLoan = loans.find((l) => l.id === destino);

  const validate = () => {
    const newErrors: Record<string, string> = {};
    if (!origen) newErrors.origen = 'Selecciona una cuenta origen';
    if (!destino) newErrors.destino = 'Selecciona un préstamo a pagar';
    ...
    if (parsedAmount > 0 && destinoLoan && parsedAmount > destinoLoan.outstandingBalance)
      newErrors.monto = 'El monto supera el saldo pendiente del préstamo';
    setErrors(newErrors);
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-sm font-semibold" style={{ color: colors.textPrimary }}>
                    {formatCurrency(destinoLoan.outstandingBalance)}
                  </p>
                </div>
                <div>
                  <p className="text-xs" style={{ color: colors.textSecondary }}>
                  ...
                  <p className="text-sm font-semibold" style={{ color: colors.textPrimary }}>
                    {formatCurrency(destinoLoan.monthlyPayment)}
                  </p>
                </div>
                <div>
                  <p className="text-xs" style={{ color: colors.textSecondary }}>
                  ...
                  <p className="text-sm font-semibold" style={{ color: colors.textPrimary }}>
                    {destinoLoan.remainingPayments} de {destinoLoan.totalPayments}
                  </p>
                </div>
              </div>
            </div>
        ...
        >
          ...
          >
            ...
            <div className="py-4 space-y-3">
              ...
              <div className="flex justify-between gap-4">
                <span style={{ color: colors.textSecondary }}>Préstamo a pagar</span>
                <span className="font-semibold text-right" style={{ color: colors.textPrimary }}>
                  {destinoLoan?.nickname || destinoLoan?.purpose}
                </span>
              </div>
              {concepto && (
                <div className="flex justify-between gap-4">
